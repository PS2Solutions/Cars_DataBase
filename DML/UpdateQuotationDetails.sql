...
IN `CustomerID` INT(10), IN `Amount` decimal(10,3), IN `Status` tinyint(1),
 IN `Address1` VARCHAR(100), IN `Address2` VARCHAR(100),IN `QuotationTypeID` INT(10), IN `ContractID` INT(10), IN `LaborCharge`  decimal(10,3), IN `CreatedDate` Datetime, OUT `Response` INT) 
 BEGIN
	UPDATE 
        quotations 
    SET
